...
pub fn shows_get((state, req): (State<AppState>, Path<GetShow>)) -> Box<Future<Item = HttpResponse, Error = WebError>> {
    Box::new(get_entire_show(req.id, state.db_pool.clone())
        .and_then(|show| {
            match show {
...
pub fn shows_all(state: State<AppState>) -> Box<Future<Item = HttpResponse, Error = WebError>> {
    Box::new(get_all_shows(state.db_pool.clone())
        .and_then(|shows| {
            match shows {
...
pub fn shows_add((state, form): (State<AppState>, Form<AddShow>))
